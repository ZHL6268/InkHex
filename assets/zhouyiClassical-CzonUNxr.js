...
《周易》又名《易經》，自漢代以後，一般所見都是「經傳」合一：也就是原本 的經文，與易傳（十翼）是合在一起的。 在體例上，每卦的卦辭或爻辭（也就是原始的經文）之後，都會附有「《象》曰」 及「《彖》曰」，此即為《象傳》及《彖傳》的文字。 另外在乾卦和坤卦兩卦之後則有「文言曰」，此即《文言傳》的文字。《繫辭上》、《繫辭下》、《說卦》、《序卦》、《雜卦》則附於六十四卦之後。易學網所收錄的《周易》全文，是以王弼《周易注》為底本加以校對與編輯，若有任何問題或疑義，歡迎與易學網連絡。
...
`;let i=null;function C(){var o,r,l,m;if(i)return i;const t={},x=/\n(\d{1,2})\s+([^\n]+?)(?=\n\d{1,2}\s+|$)/g,$=`
${g}`;let e;for(;e=x.exec($);){const a=Number(e[1]),s=e[2].trim(),n=(((o=s.split(`
`)[0])==null?void 0:o.trim())??"").match(/^([^，。,]+)[，,](.+)$/),f=((r=n==null?void 0:n[1])==null?void 0:r.trim())??"",u=((l=n==null?void 0:n[2])==null?void 0:l.trim())??"",c=s.match(/《象》曰：([^\n]+)/),b=s.matchAll(/(初九|初六|九二|六二|九三|六三|九四|六四|九五|六五|上九|上六|用九|用六)，([^\n]+)/g),h={};for(const p of b)h[p[1]]=p[2].trim();!f||!u||(t[a]={number:a,name:f,judgment:u,image:(m=c==null?void 0:c[1])==null?void 0:m.trim(),lines:h})}return i=t,t}export{C as getZhouyiClassicalMap};
